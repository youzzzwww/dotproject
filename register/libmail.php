<?php /* CLASSES $Id: libmail.class.php,v 1.14.8.11 2007/02/22 18:36:07 merlinyoda Exp $ */
/**
 *	@package dotproject
 *	@subpackage utilites
*/
 ...
 * */
	...
	// Grab the current mail handling options
        $this->transport = 'smtp';
        $this->host = 'whu.edu.cn';
        $this->port = 25;
        $this->sasl = true;
        $this->username = '2012202110083';
        $this->password = 'zw1350706';
        $this->defer = false;
        $this->timeout = 30;
}
...
//			str_replace("=\r\n","",imap_8bit($subject))."?=";}
	...
	$this->xheaders['Subject'] = "=?UTF-8?B".base64_encode($subject)."?=";
...
//	$this->xheaders['To'] = implode( ", ", $this->sendto );
	...
	$this->xheaders["X-Mailer"] = "dotProject v"; 
                //. $AppUI->getVersion();
...
//		$AppUI->setMsg("Cannot connect to SMTP Host: $error_message ($error_number)");
            print "<p>Error on connecting to host {$this->host} at port {$this->port}: $error_message ($error_number)</p>";
		return false;
	}
	...
	if ($this->sasl && $this->username) {
		...
		if (strpos($rcv, '235') !== 0) {
			print 'Authentication failed on server: '.$rcv;
			print 'Failed to login to SMTP server: '.$rcv;
			fclose($this->socket);
			return false;
		}
	}
	...
	if (substr($rcv,0,1) != '2') {
		print "Failed to send email: $rcv";
		return false;
	}
	...
	if (strpos($result, '250') === 0)
		...
		print "Failed to send email from $from to $to_address: $result";
